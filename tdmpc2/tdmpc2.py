...
class TDMPC2(torch.nn.Module):
    ...
    def _plan(self, obs, t0=False, eval_mode=False, task=None):
        ...
        if self.cfg.num_pi_trajs > 0:
            ...
            _z = z.repeat(self.cfg.num_pi_trajs, *([1] * z.ndim))
            print("_z: ", _z.shape)
            print("pi_actions: ", pi_actions.shape)
        ...
        z = z.repeat(self.cfg.num_samples, *([1] * z.ndim))
        mean = torch.zeros(
            self.cfg.horizon, self.cfg.num_envs, self.cfg.action_dim, device=self.device
        )
        std = torch.full(
            (self.cfg.horizon, self.cfg.num_envs, self.cfg.action_dim),
            self.cfg.max_std,
            dtype=torch.float,
            device=self.device,
        )
