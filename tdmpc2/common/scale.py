...
class RunningScale(torch.nn.Module):
	...
	def __init__(self, cfg):
		super().__init__()
		self.cfg = cfg
		self.value = Buffer(torch.ones(1, dtype=torch.float32, device=torch.device(cfg.device)))
		self._percentiles = Buffer(torch.tensor([5, 95], dtype=torch.float32, device=torch.device(cfg.device)))
